...
interface PositionRowProps {
  ...
  onClose?: (position: PositionWithMarketData) => void;
  logoUrl?: string;
}

export default function PositionRow({ position, onEdit, onDelete, onClose, logoUrl }: PositionRowProps) {
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-3 rounded-xl hover:bg-white/[0.03] transition-colors min-w-[860px]">
      ...
      <div className="flex items-center gap-1 ml-auto">
        {onClose && (
          <GlassButton variant="ghost" size="sm" onClick={() => onClose(position)} title="Close position (record sale)">
